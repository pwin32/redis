...
This is an unofficial Windows x64 integration of the Redis 7.2.15 behavior
...
reports `7.2.15`; the canonical `mingw-7.2` branch is the maintenance line.
Windows revision 1 is identified by tag `v7.2.15-windows.1` and archive
`Redis-x64-7.2.15-mingw-r1.zip`. The historical `v7.2.14` and
`v7.2.14-windows.1` tags and their corresponding
`Redis-x64-7.2.14-mingw.zip` and `Redis-x64-7.2.14-mingw-r1.zip` archives
remain immutable.

This guide documents behavior that differs from the original Redis 7.2.15
...
follows upstream Redis 7.2.15. See `00-RELEASENOTES` for the complete upstream
...
long persistence soak, and exact service-restart gates are the Redis 7.2.15
release gates. The historical Redis 7.2.14 archives remain immutable.

### Upstream security-fix provenance

The 7.2.15 source baseline includes the applicable upstream security fixes for
Lua parser/runtime isolation, blocked-client eviction, RESTORE/RDB validation,
and full-sync while a yielding script or Function is running. It also rejects
a crafted stream `RESTORE` payload when more than one consumer claims the same
NACK, preventing the resulting use-after-free. The Windows full-sync path
retains an additional IOCP disconnect cleanup guard in `12c1730f7`; do not
replace it with the unadapted POSIX patch. XACKDEL-specific fixes are not
applicable because that command is absent from this 7.2 line.
...
| Area | Original Redis 7.2.15 on Linux/POSIX | This Windows port |
| --- | --- | --- |
...
| Setting | Packaged Windows examples | Upstream 7.2.15 example/default | Notes |
| --- | --- | --- | --- |
...
Atomic replacement is same-volume only and uses write-through `MoveFileExW`;
Windows has no direct equivalent of POSIX directory `fsync()`. Storage caches
and transient sharing
...
## 2026-08-17 Windows revision 1 hardening and qualification

- The native test launcher no longer reads another process's PEB or trusts its
  mutable command-line text to identify a QFork child. It now accepts only an
  exact executable-image match among direct children of the owned server,
  retries briefly while Windows publishes the new process, and fails closed if
  more than one matching child exists. Test cleanup therefore uses the same
  executable ownership boundary documented for service and process auditing.
- MinGW builds now discard generated jemalloc state when the committed
  allocator tree changes across maintenance branches. The checked-in jemalloc
  `configure` script is not regenerated merely because checkout timestamps
  differ, and release packaging always starts with `distclean` and aborts if a
  build modifies tracked source. `BUILDINFO.txt` correctly identifies this
  line's `jemalloc-5.3.0-redis` Windows QFork tree rather than the separate 6.2
  line's jemalloc 5.2.1 tree.
- A clean source checkpoint built Redis 7.2.15 with the repository MinGW64
  toolchain; both legacy and modern Windows interop tests passed and all Module
  API test binaries were rebuilt. The authoritative root rerun passed all
  required units, including replication and the Windows IOCP/regression units.
  An earlier root run had one 1,517 ms host-scheduling outlier in a 1,150 ms
  BLPOP timing assertion; the complete list unit then passed five consecutive
  reruns and the unchanged assertion passed in the full rerun, so its upstream
  regression-detection ceiling was not weakened.
- The complete Cluster and Sentinel suites each finished with
  `GOOD! No errors.`; all Module API units passed. The isolated
  `RedisPortTest` service restart/Event Log gate passed and removed the service
  afterward; every post-gate audit found zero repository-owned Redis
  processes. Visual Studio/MSI,
  clean-machine, antivirus, signing, and reboot/preshutdown testing remain
  outside this MinGW release matrix.

### Publication status

The internal qualification work covered the source, package, extraction,
service, persistence, and Windows behavior gates. Internal source identifiers,
artifact checksums, and hosting operations are intentionally omitted from the
tracked guide. Public release tags and archives are deferred until a CI job
rebuilds and verifies the selected source revision.
