...
test "Cluster replicas are synchronized before reset" {
    if {$::tcl_platform(platform) eq "windows"} {
        ...
        # its master.  Wait for every replica before resetting any node: the
        # per-node check below is too late when an earlier master is still
        # serving that synchronization.
        foreach_redis_id id {
