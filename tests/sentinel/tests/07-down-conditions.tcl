...
test "SDOWN is triggered by misconfigured instance replying with errors" {
    ...
    # atomic rename step.  The old test relied on / being unwritable, but
    # that assumption is false on Windows CI runners (and for privileged
    # Unix runners).  Keep the server in its normal directory so the test
    # remains valid with request logging enabled as well.
