...
start_server {overrides {appendonly no auto-aof-rewrite-percentage 0}} {
    ...
    test {Preload file validates prefix, path, and extension} {
        ...
        if {$::tcl_platform(platform) eq "windows"} {
            set drive [string range [file normalize [pwd]] 0 1]
            set absolute_no_extension "${drive}/tmp/preload-file-no-extension"
        } else {
            set absolute_no_extension "/tmp/preload-file-no-extension"
        }
        catch {exec $::redis_server_path --port 0 --preload-file "aof:$absolute_no_extension"} err
        assert_match {*preload-file must end with an extension*} $err
        catch {exec $::redis_server_path --port 0 --preload-file "rdb:$absolute_no_extension"} err
        assert_match {*preload-file must end with an extension*} $err
        catch {exec $::redis_server_path --port 0 --preload-file "invalid:$absolute_no_extension.rdb"} err
        assert_match {*argument must be in the format*} $err
        if {$::tcl_platform(platform) eq "windows"} {
            set invalid_paths [list \
                "aof:${drive}//tmp/foo.aof" \
                "aof:${drive}/tmp/./foo.aof" \
                "aof:${drive}/tmp/../foo.aof" \
                "aof:${drive}/tmp//foo.aof"]
        } else {
        ...
        foreach path $invalid_paths {
            catch {exec $::redis_server_path --port 0 --preload-file $path} err
    ...
    test {backupdirname rejects paths at startup} {
        ...
        catch {exec $::redis_server_path --backupdirname /tmp/mybackup} err
        assert_match {*backupdirname can't be a path*} $err
        catch {exec $::redis_server_path --backupdirname nested/mybackup} err
        assert_match {*backupdirname can't be a path*} $err
    }
    ...
    test {BACKUP LIST returns absolute paths for a relative backupdirname} {
        ...
        assert_equal [file normalize $bdir] [file normalize [file dirname [lindex $files 0]]]
        assert {[file exists [lindex $files 0]]}
        ...
        foreach f $files {
            ...
            assert_equal [file normalize $bdir] [file normalize [file dirname $f]]
            assert {[file exists $f]}
        }
        assert_equal "OK" [r backup cleanup]
    }
