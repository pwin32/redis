...
start_multiple_servers 3 [list overrides $base_conf] {
    ...
    test "use previous hostip in \"cluster-preferred-endpoint-type unknown-endpoint\" mode" {
        ...
        assert_equal "[exec $::redis_cli_path -h 127.0.0.1 -p [srv 0 port] -c set foo bar]" {OK}
        assert_match "[exec $::redis_cli_path -h 127.0.0.1 -p [srv 0 port] -c get foo]" {bar}
...
start_multiple_servers 5 [list overrides $base_conf] {
    ...
    test {Functions are added to new node on redis-cli cluster add-node} {
        ...
        exec $::redis_cli_path --cluster-yes --cluster call 127.0.0.1:[srv 0 port] \
                           FUNCTION LOAD {#!lua name=TEST
                               redis.register_function('test', function() return 'hello' end)
                           }

        # adding node to the cluster
        exec $::redis_cli_path --cluster-yes --cluster add-node \
...
test {Migrate the last slot away from a node using redis-cli} {
    start_multiple_servers 4 [list overrides $base_conf] {
        ...
        # Insert some data
        assert_equal OK [exec $::redis_cli_path -c -p [srv 0 port] SET foo bar]
        set slot [exec $::redis_cli_path -c -p [srv 0 port] CLUSTER KEYSLOT foo]
        ...
        wait_for_condition 1000 50 {
            ...
            [catch {exec $::redis_cli_path --cluster check 127.0.0.1:[srv -1 port]}] == 0 &&
            [catch {exec $::redis_cli_path --cluster check 127.0.0.1:[srv -2 port]}] == 0 &&
            [catch {exec $::redis_cli_path --cluster check 127.0.0.1:[srv -3 port]}] == 0 &&
            [CI 0 cluster_state] eq {ok} &&
            [CI 1 cluster_state] eq {ok} &&
            [CI 2 cluster_state] eq {ok} &&
            [CI 3 cluster_state] eq {ok}
        ...
        exec $::redis_cli_path --cluster reshard 127.0.0.1:[srv -3 port] \
...
start_cluster 3 0 {} {
    test {Rebalance without --user and without -a should succeed} {
        ...
        set master0_id [R 0 CLUSTER MYID]
            ...
            exec $::redis_cli_path --cluster-yes --cluster rebalance \
                               127.0.0.1:[srv 0 port] \
...
start_server [list overrides [list cluster-enabled yes cluster-node-timeout 1 cluster-port [find_available_port $::baseport $::portcount]]] {
    ...
    test "redis-cli -4 --cluster create using $ip_or_localhost with cluster-port" {
        exec $::redis_cli_path -4 --cluster-yes --cluster create \
    ...
    test "redis-cli -4 --cluster add-node using $ip_or_localhost with cluster-port" {
        ...
        exec $::redis_cli_path -4 --cluster-yes --cluster add-node \
