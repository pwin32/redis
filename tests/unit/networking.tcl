...
start_server {config "minimal.conf" tags {"external:skip"}} {
    test {Default bind address configuration handling} {
        ...
        if {$::tcl_platform(platform) eq "windows"} {
            ...
            # restart and uses it to restore the TCP listener.  The Windows
            # port has no Unix socket, so an empty bind leaves no listener and
            # startup must fail.  Verify that persisted behavior with a
            # separate probe while the original client is still connected.
