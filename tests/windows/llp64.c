...
_Static_assert(sizeof(((dict *)0)->ht_used[0]) == sizeof(uint64_t),
...
_Static_assert(sizeof(((struct redisMemOverhead *)0)->db_dict_rehashing_count) == sizeof(uint64_t),
               "Win64 rehashing database counts must be 64-bit");
_Static_assert(sizeof(((struct redisServer *)0)->aof_delayed_fsync) == sizeof(uint64_t),
               "Win64 delayed-fsync counters must be 64-bit");
_Static_assert(sizeof(((struct redisServer *)0)->reply_buffer_peak_reset_time) == sizeof(int64_t),
               "Win64 reply-buffer reset times must be 64-bit");
...
int win32_llp64_interop_test(void) {
    ...
    struct redisMemOverhead memory_overhead = {0};
    ...
    memory_overhead.db_dict_rehashing_count = high + 7;
    server_widths.aof_delayed_fsync = high + 8;
    server_widths.reply_buffer_peak_reset_time = (mstime_t)(high + 9);
    function_symbol.pointer = (uintptr_t)&function_symbol;

    return DICTHT_SIZE(33) == (UINT64_C(1) << 33) &&
           dictNextExpForSize((UINT64_C(1) << 32) + 1) == 33 &&
           d.ht_used[0] == high && d.rehashidx == (int64_t)high &&
