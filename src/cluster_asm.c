...
/* Shut down the underlying socket without invoking the connection type's
 * shutdown callback. The ASM fail points intentionally keep the connection
 * object alive, and TLS shutdown may otherwise free or block on stale state. */
static inline void asmRawConnShutdown(connection *conn) {
#ifdef _WIN32
...
sds asmCatInfoString(sds info) {
    ...
    return sdscatprintf(info ? info : sdsempty(),
                        ...
                        "cluster_slot_migration_active_trim_running:%llu\r\n"
                        ...
                        active_tasks,
                        (unsigned long long)listLength(asmManager->active_trim_jobs),
...
void asmFeedMigrationClient(robj **argv, int argc) {
    ...
    for (int i = 0; i < argc; i++) {
        if (!sdsEncodedObject(argv[i])) {
            ...
            robj *old = argv[i];
            argv[i] = createStringObjectFromLongLongWithSds((long long)(intptr_t)old->ptr);
            decrRefCount(old);
        }
    }
...
void asmSlotSnapshotAndStreamStart(struct asmTask *task) {
    ...
    if (unlikely(asmDebugIsFailPointActive(ASM_MIGRATE_RDB_CHANNEL, task->state))) {
        asmRawConnShutdown(task->rdb_channel_client->conn);
        return;
    }
            ...
            for (int k = sr->start; k <= sr->end; k++) {
                ...
                while ((de = kvstoreDictIteratorNext(&kvs_di)) != NULL) {
                    ...
                    if (!send_slot_info) {
                        /* Format slot info */
                        char buf[128];
                        int len = snprintf(buf, sizeof(buf), "%d:%llu:%llu",
                                    k,
                                    (unsigned long long)kvstoreDictSize(db->keys, k),
                                    (unsigned long long)kvstoreDictSize(db->expires, k));
...
void asmSyncBufferStreamToDb(asmTask *task) {
    ...
    if (ret == C_OK) {
        ...
        if (unlikely(asmDebugIsFailPointActive(ASM_IMPORT_MAIN_CHANNEL, task->state)))
            asmRawConnShutdown(task->main_channel_conn); /* Simulate a failure */
