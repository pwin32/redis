...
static int luaAllocFlags(void *ud) {
#if defined(_WIN32) && !defined(NO_QFORKIMPL)
    UNUSED(ud);
    /* QFork's external heap and the explicit Lua tcache are incompatible in
     * this 4 MiB-page configuration: the tcache fill metadata is corrupted
     * before the first script compiles. Keep the dedicated Lua arena, but use
     * direct arena allocations on QFork Windows builds. */
    return MALLOCX_ARENA(server.lua_arena) | MALLOCX_TCACHE_NONE;
#else
    unsigned int tcache = (unsigned int)(uintptr_t)ud;
    return MALLOCX_ARENA(server.lua_arena) | MALLOCX_TCACHE(tcache);
#endif
}
...
static void *luaAlloc(void *ud, void *ptr, size_t osize, size_t nsize) {
    ...
    int flags = luaAllocFlags(ud);
    if (nsize == 0) {
        zfree_with_flags(ptr, flags);
        return NULL;
    } else {
        return zrealloc_with_flags(ptr, nsize, flags);
    }
}
