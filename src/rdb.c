...
ssize_t rdbSaveStringObject(rio *rdb, robj *obj) {
    ...
    if (obj->encoding == OBJ_ENCODING_INT) {
        return rdbSaveLongLongAsStringObject(rdb,(long long)(intptr_t)obj->ptr);
    } else {
...
ssize_t rdbSaveDb(rio *rdb, int dbid, int rdbflags, uint64_t *key_counter, unsigned long long *skipped) {
    dictEntry *de;
    ssize_t written = 0;
    ssize_t res;
    size_t oldsize = 0;
    ...
    int track_alloc_size = server.memory_tracking_enabled;
#ifdef _WIN32
    /* A QFork child maps the parent's Redis heap without inheriting the
     * parent's jemalloc metadata.  Allocation-size accounting is parent-side
     * state and changes made here are discarded when the child exits, so do
     * not query inherited kvobj allocations while serializing the snapshot. */
    if (server.in_fork_child) track_alloc_size = 0;
#endif
...
int rdbSaveRio(int req, rio *rdb, int *error, int rdbflags, rdbSaveInfo *rsi) {
    ...
    serverLog(LL_NOTICE, "BGSAVE done, %llu keys saved, %llu keys skipped, %zu bytes written.",
              (unsigned long long)key_counter, skipped, rdb->processed_bytes);
    return C_OK;
...
int rdbSave(int req, char *filename, rdbSaveInfo *rsi, int rdbflags) {
    ...
    WIN32_ONLY(tmpfile[sizeof(tmpfile) - 1] = '\0';)
...
int rdbSaveBackground(int req, char *filename, rdbSaveInfo *rsi, int rdbflags) {
    ...
    childpid = BeginForkOperation_Rdb(req, filename, rsi,
                                      rsi ? sizeof(*rsi) : 0, rdbflags,
...
robj *rdbLoadObject(int rdbtype, rio *rdb, sds key, int dbid, int *error)
{
    ...
    if (rdbtype == RDB_TYPE_STRING) {
        ...
        if (tmpl == NULL) {
            rdbReportCorruptRDB("Invalid hash template ID %llu", (unsigned long long)template_id);
            return NULL;
        }
        unsigned long long field_count = tmpl->field_count;
        ...
        for (uint64_t i = 0; i < count; i++) {
            ...
            if (type_tag == AR_RDB_TAG_INT) {
                ...
                if (len > AR_SMALLSTR_MAXLEN) {
                    sdsfree(str);
                    decrRefCount(o);
                    rdbReportCorruptRDB("Invalid small string length %llu in array",
                                        (unsigned long long)len);
                    return NULL;
                }
...
static int rdbLoadRioWithLoadingCtxInternal(rio *rdb, int rdbflags, rdbSaveInfo *rsi, rdbLoadingCtx *rdb_loading_ctx) {
    ...
    while(1) {
        ...
        if (should_expand_db) {
            ...
            should_expand_db = 0;
            serverLog(LL_VERBOSE, "DB %d resized: %llu key buckets, %llu expire buckets",
                        db->id, (unsigned long long)kvstoreBuckets(db->keys),
                        (unsigned long long)kvstoreBuckets(db->expires));
        }
...
int rdbLoadWithEmptyFunc(char *filename, rdbSaveInfo *rsi, int rdbflags, void (*emptyDbFunc)(void)) {
    ...
    if (retval == C_OK && !(rdbflags & RDBFLAGS_KEEP_CACHE)) {
        ...
        int rdb_fd = open(filename, O_RDONLY, 0);
...
int rdbSaveToSlavesSockets(int req, rdbSaveInfo *rsi) {
    ...
    int use_parent_pipe = !rdb_channel;
#ifdef _WIN32
    /* QFork duplicates the established FDAPI pipe endpoints into the child.
     * Keep RDB-channel delivery on that qualified path and let the parent
     * forward the snapshot to the dedicated replica connection. */
    use_parent_pipe = 1;
#endif
...
#ifdef _WIN32
    long long start = ustime();
    childpid = BeginForkOperation_Socket(req, rsi,
                                         rsi ? sizeof(*rsi) : 0,
                                         rdb_pipe_write,
                                         safe_to_exit_pipe,
                                         &server,
    ...
    if ((childpid = redisFork(CHILD_TYPE_RDB)) == 0) {
        ...
        if (!use_parent_pipe) {
            ...
            rioFreeFd(&rdb);
            /* Wake up the reader and hold the child until the parent drains
             * the RDB stream. */
            close(rdb_pipe_write);
            close(server.rdb_child_exit_pipe);
    ...
    /* Parent */
    if (use_parent_pipe) close(safe_to_exit_pipe);
    if (childpid == -1) {
        serverLog(LL_WARNING,"Can't save in background: fork: %s",
            IF_WIN32(wsa_strerror(errno),strerror(errno)));
        ...
        serverLog(LL_NOTICE, "Background RDB transfer started by pid %ld to %s",
                  (long)childpid,
                  use_parent_pipe ? "parent process pipe" :
                  slots_req ? "slot migration destination socket" : "replica socket");
        server.rdb_save_time_start = time(NULL);
