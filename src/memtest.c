...
int memtest_addressing(uintptr_t *l, size_t bytes, int interactive) {
...
void memtest_fill_random(uintptr_t *l, size_t bytes, int interactive) {
    uintptr_t step = 4096/sizeof(uintptr_t);
    uintptr_t words = bytes/sizeof(uintptr_t)/2;
    uintptr_t iwords = words/step;  /* words per iteration */
    uintptr_t off, w, *l1, *l2;
...
void memtest_fill_value(uintptr_t *l, size_t bytes, uintptr_t v1,
                        uintptr_t v2, char sym, int interactive)
{
    uintptr_t step = 4096/sizeof(uintptr_t);
    uintptr_t words = bytes/sizeof(uintptr_t)/2;
    uintptr_t iwords = words/step;  /* words per iteration */
    uintptr_t off, w, *l1, *l2, v;
...
int memtest_compare(uintptr_t *l, size_t bytes, int interactive) {
    uintptr_t words = bytes/sizeof(uintptr_t)/2;
    uintptr_t w, *l1, *l2;
...
int memtest_compare_times(uintptr_t *m, size_t bytes, int pass, int times,
                          int interactive)
{
    int j;
    int errors = 0;
...
int memtest_test(uintptr_t *m, size_t bytes, int passes, int interactive) {
    int pass = 0;
    int errors = 0;
    ...
    while (pass != passes) {
        ...
        memtest_fill_value(m,bytes,0,(uintptr_t)-1,'S',interactive);
        if (interactive) memtest_progress_end();
        errors += memtest_compare_times(m,bytes,pass,4,interactive);
...
#define MEMTEST_BACKUP_WORDS (1024*(1024/sizeof(uintptr_t)))
...
int memtest_preserving_test(void *memory, size_t bytes, int passes) {
    ...
    uintptr_t *p = m;
    uintptr_t *end = (uintptr_t*)(((unsigned char*)m)+(bytes-MEMTEST_DECACHE_SIZE));
    size_t left = bytes;
    int errors = 0;
    ...
    while(left) {
        ...
        while(pass != passes) {
            ...
            memtest_fill_value(p,len,0,(uintptr_t)-1,'S',0);
            if (bytes >= MEMTEST_DECACHE_SIZE) {
                memtest_compare_times(m,MEMTEST_DECACHE_SIZE,pass,1,0);
                memtest_compare_times(end,MEMTEST_DECACHE_SIZE,pass,1,0);
            }
