...
#else
/* Windows has no POSIX signal-handler logging path. Route raw handler output
 * through the native logger so 7.4's shared crash/thread code still links. */
void serverLogRawFromHandler(int level, const char *msg) {
    _serverLog(level, "%s", msg);
}
#endif
...
void call(client *c, int flags) {
    ...
    /* Setting the CLIENT_REEXECUTING_COMMAND flag so that during the actual
     * processing of the command proc, the client is aware that it is being
     * re-processed. */
    if (reprocessing_command) c->flags |= CLIENT_REEXECUTING_COMMAND;
...
sds genRedisInfoString(dict *section_dict, int all_sections, int everything) {
    ...
    if (all_sections || (dictFind(section_dict,"server") != NULL)) {
        ...
        const char *os_machine = "";
#else
        const char *os_name = name.sysname;
        const char *os_release = name.release;
        const char *os_machine = name.machine;
#endif
...
#ifdef _WIN32
    /* The Windows port remains deliberately single-threaded for client I/O.
     * Reject unsupported modes before initServer() creates BIO/event-loop
     * state so a bad config cannot leave worker threads behind. */
