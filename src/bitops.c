...
long long redisPopcount(void *s, long count) {
     ...
     * otherwise align to 32-bit. uintptr_t is required for Windows LLP64. */
    int align = use_popcnt ? 7 : 3;
    while ((uintptr_t)p & align && count) {
        bits += bitsinbyte[*p++];
        count--;
    }
...
void getbitCommand(client *c) {
    ...
    if (sdsEncodedObject(kv)) {
        ...
        if (byte < (size_t)ll2string(llbuf,sizeof(llbuf),(long long)(intptr_t)kv->ptr))
...
void bitopCommand(client *c) {
    ...
    if (maxlen) {
        ...
        /* If no AVX2 path was used, fall back to a word-at-a-time fast path
         * that is still much better than the byte-by-byte loop below. On ARM
         * we skip this since it would cause GCC to emit multiple-word
         * load/store operations not supported even on ARM >= v6. */
