...
void bioPipeReadJobCompList(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    } while (nread == (ssize_t)sizeof(buf));

    if (nread < 0 && errno != EAGAIN && errno != EWOULDBLOCK) {
        serverLog(LL_WARNING,
            "Error reading the background I/O completion pipe: %s",
            wsa_strerror(errno));
    }
    /* IOCP read readiness is one-shot.  Keep the completion pipe armed after
     * draining the notification, otherwise the next lazy-free completion can
     * remain queued forever while its waiting client blocks. */
    if (WSIOCP_QueueNextRead(fd) != 0) {
        serverLog(LL_WARNING,
            "Error rearming the background I/O completion pipe: %s",
            wsa_strerror(errno));
    }
#else
