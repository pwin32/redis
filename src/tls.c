...
#ifdef _WIN32
    if (need_write) {
        /* Keep the AE-level marker in sync so separate IOCP completions still
         * honor the TLS connection's logical write barrier. */
        int barrier = conn->c.write_handler &&
                      (conn->c.flags & CONN_FLAG_WRITE_BARRIER);
        aeSetFileEventBarrier(conn->c.el, conn->c.fd, barrier);
    }
#endif
}
...
static void tlsHandleEvent(tls_connection *conn, int mask) {
    ...
    /* The event loop may have been unbound during event processing. */
    if (conn->c.el) {
        updateSSLEvent(conn);
#ifdef _WIN32
        tlsRearmEvents(conn);
