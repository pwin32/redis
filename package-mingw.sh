#!/usr/bin/env bash
# Build the Redis 8.8.1 Windows core MinGW64 revision 2 package.
...
if [[ "$version" != "8.8.1" ]]; then
    echo "error: package-mingw.sh is restricted to Redis 8.8.1; found $version" >&2
    exit 1
fi

windows_revision="${WINDOWS_PACKAGE_REVISION:-2}"
if [[ "$windows_revision" != "2" ]]; then
    echo "error: Redis 8.8.1 packaging requires WINDOWS_PACKAGE_REVISION=2" >&2
    exit 1
fi
...
if [[ "$gcc_version" != "16.1.0" ]]; then
    echo "error: Redis 8.8.1 revision 2 requires GCC 16.1.0; found $gcc_version" >&2
    exit 1
fi
...
BUILD_BUNDLED_MODULES=no SKIP_VEC_SETS=yes ./build-mingw.sh distclean
BUILD_BUNDLED_MODULES=no SKIP_VEC_SETS=yes ./build-mingw.sh -j"$jobs"
...
install -m 0644 WINDOWS-8.8-CHANGES.md "$stage_dir/WINDOWS-8.8-CHANGES.md"
