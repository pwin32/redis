#!/usr/bin/env bash
# Build the Redis 6.2.23 Windows MinGW64 revision 1 package.
...
if [[ "$version" != "6.2.23" ]]; then
    echo "error: package-mingw.sh is restricted to Redis 6.2.23; found $version" >&2
    exit 1
fi

windows_revision="${WINDOWS_PACKAGE_REVISION:-1}"
if [[ "$windows_revision" != "1" ]]; then
    echo "error: Redis 6.2.23 packaging requires WINDOWS_PACKAGE_REVISION=1" >&2
    exit 1
fi
...
if [[ "$gcc_version" != "16.1.0" ]]; then
    echo "error: Redis 6.2.23 revision 1 requires GCC 16.1.0; found $gcc_version" >&2
    exit 1
fi
...
install -m 0644 WINDOWS-6.2-CHANGES.md "$stage_dir/WINDOWS-6.2-CHANGES.md"
...
# These programs select their role from argv[0]. Copy the already stripped
# server so the aliases are byte-identical and packaging is deterministic.
