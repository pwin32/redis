...
This is an unofficial Windows x64 Redis 8.8.2 revision 1 core build. It is built
...
This package reports Redis 8.8.2. A public CI release uses
`Redis-x64-8.8.2-mingw-r1.zip` and the matching revision tag; no tag or
...
service action as the first argument:
    ...
    redis-server.exe --service-install --service-name Redis882 redis.windows-service.conf
    redis-server.exe --service-start --service-name Redis882
    redis-server.exe --service-stop --service-name Redis882
    redis-server.exe --service-uninstall --service-name Redis882
